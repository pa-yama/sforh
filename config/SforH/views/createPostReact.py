...
def fromHeaderFunc(request):
    ...
    #prevScreenName = request.data.get('prevScreenName')
    
    #print('prevScreenName')
    #print(prevScreenName)
...
@api_view(['POST'])
@renderer_classes([JSONRenderer])  # JSONレンダラーを指定
def fromHeaderCreatePostFunc(request):
    
    print("fromHeaderCreatePostFunc")
    ...
    print(request.data.get('problem'))
    ...
    object = Post_Tmp.objects.create(
        ...
        text_probrem = request.data.get('problem'),
    ...
    #redirect('search-post')#検索画面をマージしたらこっち
...
@api_view(['POST'])
@renderer_classes([JSONRenderer])  # JSONレンダラーを指定
def fromHeaderSavePostFunc(request):
    print("新規登録確認")
    template_name = 'SforH/create_post.html'
...
@api_view(['GET'])
@renderer_classes([JSONRenderer])  # JSONレンダラーを指定
def fromDetailFunc(request, post_id):
    
    #投稿テーブルから既存の内容を取得して、Jsonデータとして送る。
...
@api_view(['POST'])
@renderer_classes([JSONRenderer])  # JSONレンダラーを指定
def fromDetailCreatePostFunc(request, post_id):
    ...
    object.tag = request.data.get('tag')
    object.text_probrem = request.data.get('problem')
    ...
    #投稿テーブルのデータを更新。
    object = Post.objects.get(post_id=postId)
    object.title = request.data.get('title')
    object.tag = request.data.get('tag')
    object.text_probrem = request.data.get('problem')
    #object.text_solution = form.cleaned_data['text_solution']
    ...
    return redirect('create-post-edit', post_id=post_id)
...
@api_view(['POST'])
@renderer_classes([JSONRenderer])  # JSONレンダラーを指定
def fromDetailSavePostFunc(request, post_id):
